"""Run paired ABC LCO selection and locked formal Unknown test."""
...
def resolve(path, stack=()):
    ...
    p = p.resolve()
    if p in stack:
        raise ValueError("cyclic ABC config inheritance")
    raw = json.loads(p.read_text(encoding="utf-8"))
    base = resolve(raw["base_config"], (*stack, p)) if "base_config" in raw else {}
    cfg = {**base, **raw}
    for key in ("oracle_root", "wisig_pkl", "mother_config", "final_checkpoint"):
...
def main():
    ...
    parser.add_argument("--config", default="configs/experiments/stage11_abc_oracle.json")
    ...
    if args.dry_run:
        print(json.dumps({"oracle": oracle, "wisig": wisig}, indent=2,
                         ensure_ascii=False))
        return
    from maros_stage11.experiment import run_pair
    output = ROOT / "results" / "stage11_abc"
